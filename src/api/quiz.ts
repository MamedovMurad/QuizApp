...
const createCategories = (title:string) => {
  return api.post("/admin/categories",{title})
}
...
const getAllQuizes = async (page: number = 1, pageSize: number = 25, type = "", search:string,cat:string|number): Promise<PaginatedResponse<QuestionResponse>> => {
  return api.get(`/admin/questions?page=${page}&limit=${pageSize}&type=${type}&q=${search}&category=${cat}`).then(res => res.data);
}
